// Suppressed gun variants: give each via __hc.gun(id), screenshot the held viewmodel.
...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(`__hc.cam({yaw:0,pitch:0.1})`);
    for(const id of ['revolver_suppressed','ar15_suppressed','minigun_suppressed','ar15_suppressed_dot']){
      console.log(await page.evaluate(`__hc.gun('${id}')`));
      await sleep(700);
      await page.screenshot({ path: path.join(OUT,'gun-'+id+'.png') });
      console.log('shot',id);
    }
    await browser.close();

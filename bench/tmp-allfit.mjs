...
(async()=>{ const port=await freePort();
  ...
  let b; try{ const base='http://127.0.0.1:'+port; await waitHttp(base+'/index.html');
    ...
    for(const [sl,id] of [['optic','red_dot'],['muzzle','suppressor'],['grip','foregrip'],['light','weapon_light'],['laser','laser_sight']]){
      await p.evaluate(`__hc.cmdRun('/give ${id} 1'); __hc.attFit('${sl}','${id}')`); await sleep(250);
    }
    await p.evaluate("__hc.aim(true)");
    let s=null; for(let i=0;i<16;i++){ await sleep(200); s=await p.evaluate("__hc.opticEye()"); if(s.adsT>0.99) break; }
    await sleep(600);
    await p.screenshot({path:'C:/Users/thera/AppData/Local/Temp/claude/C--Users-thera/f3f45f2f-6bb7-4d56-87a5-95b314c4601d/scratchpad/att3/05-ads-settled.png'});
    await p.screenshot({path:'C:/Users/thera/AppData/Local/Temp/claude/C--Users-thera/f3f45f2f-6bb7-4d56-87a5-95b314c4601d/scratchpad/att3/06-ads-crop.png',clip:{x:490,y:250,width:300,height:250}});
    console.log('ALL FIVE FITTED:', JSON.stringify(s));
    console.log('wearing:', JSON.stringify((await p.evaluate("__hc.attProbe()")).wearing));

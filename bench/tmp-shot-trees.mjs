...
(async () => {
  ...
  try {
    ...
    await sleep(2500);
    // teleport inland (first probe also parks the camera there), wait for chunks to stream + mesh, probe again for real counts
    const probe = await page.evaluate(`__hc.treeprobe()`);
    await sleep(9000);                       // let ALL mesh stages (opaque + deferred leaf STAGE B) complete
    await page.evaluate(`__hc.pitch && __hc.pitch(-0.15)`); await sleep(400);
    await page.screenshot({ path: path.join(OUT,'trees-lit.png') });
    console.log(JSON.stringify({ leafMeshes:probe.leafMeshes, leafVerts:probe.leafVerts, aSky:probe.aSky, oneTree:probe.oneTree, errors }));

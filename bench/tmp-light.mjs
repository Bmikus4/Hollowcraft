...
(async () => {
  ...
  try {
    ...
    await page.goto('http://127.0.0.1:'+port+'/index.html?debug=1&t=630', { waitUntil:'load', timeout:90000 });  // t=630 = night
    await page.waitForFunction(`(() => { try { return window.__hc && __hc.st().started===true; } catch(e){ return false; } })()`, { timeout:90000 });
    await sleep(5000);
    const r = await page.evaluate(`__hc.lightProbe()`);
    await sleep(2500);
    await page.screenshot({ path: path.join(OUT,'light-lantern.png') });
    console.log(JSON.stringify({ lantern:r.lantern, grid:r.grid, err:r.err, errors }, null, 1));

// One shot: stand on the south beach, face open sea — the horizon must be sky fading straight into deep blue.
...
(async()=>{
  ...
  try{
    ...
    await page.screenshot({ path: path.join(OUT,'sea-horizon.png') });
    console.log('dbg:', JSON.stringify(await page.evaluate(`__hc.horizonDbg(false,false)`))); await sleep(400);
    await page.screenshot({ path: path.join(OUT,'sea-horizon-nolayers.png') });    // bisect: BOTH horizon layers hidden

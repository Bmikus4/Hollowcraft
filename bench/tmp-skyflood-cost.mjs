// WHAT THE SKYLIGHT FLOOD COSTS THE MESHER, priced before it is added to the standing debt rather than after.
//
...
//
//   node bench/tmp-skyflood-cost.mjs
...
(async()=>{
  ...
  try{
    ...
    const pass=async(bake)=>{
      await page.evaluate(`__hc.skyFlood({on:${bake}})`);

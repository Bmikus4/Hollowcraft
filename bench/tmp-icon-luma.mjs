...
(async()=>{
  ...
  try{
    ...
    const IDS=[]; for(const t of ['wood','stone','iron','diamond']) for(const k of ['pickaxe','axe','shovel','sword']) IDS.push(t+'_'+k);
    IDS.push('wooden_spear','rusty_spear','iron_ingot','bread');
    console.log('icon brightness (mean luma of the opaque pixels, and how much of the tile they cover)');
    for(const id of IDS){
      const url=await page.evaluate(`__hc.iconURLFor(${JSON.stringify(id)})`);
      if(!url || !/^data:image\/png;base64,/.test(url)){ console.log('  '+id.padEnd(16)+' no png ('+String(url).slice(0,40)+')'); continue; }
      const img=decodePNG(Buffer.from(url.split(',')[1],'base64'));
      const ch=img.ch; let s=0,n=0,mx=0; for(let i=0;i<img.data.length;i+=ch){ if(ch>3 && img.data[i+3]<128) continue;
        const l=0.2126*img.data[i]+0.7152*img.data[i+1]+0.0722*img.data[i+2]; s+=l; n++; if(l>mx)mx=l; }
      console.log('  '+id.padEnd(16)+' mean '+String(n?(s/n).toFixed(1):'-').padStart(6)+'  max '+String(mx.toFixed(0)).padStart(4)+'  cover '+(100*n/(img.w*img.h)).toFixed(2)+'%'+'  '+img.w+'px');
    }
    await browser.close();

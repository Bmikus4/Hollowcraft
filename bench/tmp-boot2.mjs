...
(async()=>{ const port=await freePort();
  ...
  let b; try{ const base='http://127.0.0.1:'+port; await waitHttp(base+'/index.html');
    ...
    await sleep(20000);
    console.log('started', await p.evaluate("(()=>{try{return !!(window.__hc&&__hc.st().started);}catch(e){return 'no __hc: '+e.message;}})()"));
    console.log('load', await p.evaluate("(()=>{const l=document.getElementById('load');return l?l.style.display:'?';})()"));

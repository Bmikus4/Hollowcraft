...
(async()=>{
  ...
  try{
    ...
    await sleep(1200);
    const shard = await page.evaluate(`__hc.shardTest()`);
    await sleep(400); await page.screenshot({ path: path.join(OUT,'vis-shards.png') });
    console.log(JSON.stringify({ pageErrors:errors, shard }, null, 1));
    await browser.close();

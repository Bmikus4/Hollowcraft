// Held book/bible viewmodel pose: give the field guide + bible, screenshot each held.
...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(`__hc.cam?__hc.cam({yaw:0,pitch:0.05}):0`);
    for(const id of ['field_guide','bible']){
      console.log(await page.evaluate(`__hc.hold('${id}')`)); await sleep(400); console.log('VIEW', await page.evaluate('JSON.stringify(__hc.viewDbg())'));
      await sleep(900);
      await page.screenshot({ path: path.join(OUT,'book-'+id+'.png') });
      console.log('shot',id);
    }
    await browser.close();

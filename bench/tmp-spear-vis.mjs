...
(async()=>{
  ...
  try{
    ...
    const full=async()=>{ await sleep(500); return decodePNG(await page.screenshot()); };
    await page.evaluate('__hc.holdNone()'); const empty=await full();
    for(const id of ['iron_pickaxe','iron_sword','wooden_spear','rusty_spear','wood_pickaxe']){
      await page.evaluate(`__hc.hold(${JSON.stringify(id)})`);
      const dbg=await page.evaluate('__hc.viewBounds()');
      const img=await full(); let n=0;
      for(let i=0;i<img.data.length;i+=4) if(Math.abs(lum(img.data,i)-lum(empty.data,i))>=12) n++;
      console.log(id.padEnd(15)+' '+JSON.stringify(dbg));
      await page.evaluate('__hc.holdNone()'); await sleep(200); }
    await browser.close();

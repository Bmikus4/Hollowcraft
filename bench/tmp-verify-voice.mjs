// quick error-check: the god-voice speech path fires clean (audio graph builds, banner renders, no exceptions)
...
(async () => {
  ...
  try {
    ...
    const browser = await chromium.launch({ executablePath:'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe', headless:true,
      args:['--enable-gpu','--mute-audio','--autoplay-policy=no-user-gesture-required'] });
    ...
    await sleep(1500);
    await page.evaluate(`__hc.say('taunt3')`);
    await sleep(600);
    const r = await page.evaluate(`(()=>{ const w=[...document.body.children].find(d=>d.textContent&&d.textContent.includes('VESH')); return { banner:!!w }; })()`);
    await sleep(3000);
    await page.evaluate(`__hc.say('death2')`);
    await sleep(3000);
    console.log(JSON.stringify({ banner:r.banner, errors }));

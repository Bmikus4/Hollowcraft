...
(async () => {
  ...
  try {
    ...
    await sleep(2500);
    await page.evaluate(`__hc.treeprobe()`); await sleep(9000);           // teleport to a forest + stream
    const p = await page.evaluate(`__hc.treeprobe()`);                    // find a trunk
    const tr = p.oneTree && p.oneTree.trunk;
    if(tr){ await page.evaluate(`__hc.tpExact(${tr.wx}, ${tr.wz}, ${tr.h+1})`); await sleep(6000); }  // stand UNDER the canopy
    const sky = await page.evaluate(`__hc.skyprobe ? __hc.skyprobe() : 'no-skyprobe'`);
    await page.evaluate(`__hc.pitch && __hc.pitch(-0.2)`); await sleep(400);
    await page.screenshot({ path: path.join(OUT,'skydark.png') });
    console.log(JSON.stringify({ trunk:tr, skyprobe:sky, errors }));

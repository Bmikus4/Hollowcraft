...
(async () => {
  ...
  try {
    ...
    const r = await page.evaluate(`__hc.breakTest()`);
    await sleep(2500);   // let the remesh/relight run several frames
    await page.screenshot({ path: path.join(OUT,'break-after.png') });
    const st = await page.evaluate(`__hc.st()`);   // fps/day still updating?
    await sleep(1500);
    const st2 = await page.evaluate(`__hc.st()`);
    console.log(JSON.stringify({ result:r, dayMoving: st.day!==st2.day, fps:[st.fps,st2.fps], errNewSinceBreak: errors.slice(errBefore), allErrors:errors.slice(0,8) }));

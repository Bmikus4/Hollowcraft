...
(async()=>{
  ...
  try{
    ...
    // ---- A PEER, seen in the world ----
    await page.evaluate('__hc.eqUI("close")'); await sleep(500);
    for(const [tag,pk] of [['peer-nopack',false],['peer-pack',true]]){
      const st=await page.evaluate('__hc.fakePeer('+pk+',3.4)');
      await sleep(1200);
      await page.screenshot({ path: path.join(OUT,'pack-'+tag+'.png') });
      console.log('  '+tag.padEnd(14)+' '+JSON.stringify(st));
    }

// Daytime woods dark-zone repro: sweep several inland forest spots, ground-level look shots.
...
(async()=>{
  ...
  try{
    ...
    const spots=[[560,120],[600,60],[520,180],[480,90]];
    for(let i=0;i<spots.length;i++){ const [x,z]=spots[i];
      await page.evaluate(`__hc.tp(${x},${z})`); await sleep(4500);
      await page.evaluate(`__hc.cam({yaw:0.7, pitch:-0.12})`); await sleep(700);
      await page.screenshot({ path: path.join(OUT,'woods-'+i+'.png') });
      console.log('shot',i,x,z);
    }
    await browser.close();

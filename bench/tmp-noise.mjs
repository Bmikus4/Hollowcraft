...
await p.evaluate("__hc.lock(true); __hc.pinScene(); __hc.freeze(true,false)"); await sleep(1500);
console.log('bare ', JSON.stringify(await p.evaluate("(()=>{__hc.hold('bullpup'); const r=__hc.handFire('main',1,'bullpup'); return {fired:r&&r.fired, why:__hc.fireWhy(), shot:__hc.shotNoise()};})()")));
console.log('sup  ', JSON.stringify(await p.evaluate("(()=>{__hc.attFit('muzzle','suppressor'); const r=__hc.handFire('main',1,'bullpup'); return {fired:r&&r.fired, shot:__hc.shotNoise()};})()")));

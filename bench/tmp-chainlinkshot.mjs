...
(async()=>{
  ...
  try{
    ...
    const at=await page.evaluate(`(()=>{ const p=__hc.st(); const x=Math.floor(p.px), z=Math.floor(p.pz), y=Math.floor(p.py);
      __hc.chainlinkRun(x,y,z,10,'z'); return {x,y,z,py:p.py}; })()`);
    await sleep(4500);
    ...
    const cx=at.x+0.5, cz=at.z+4.5, cy=at.y+0.55;   // the wall's middle: it spans z..z+7 and stands 2 courses high

    // CLOSE, face on — the wall spans Z so the vantage stands off in X
    await aim(cx+5.5, cz, cy+0.4, cx, cz, cy); await sleep(1600);
    fs.writeFileSync(path.join(OUT,'chainlink-close.png'), await page.screenshot());
    // FAR — does it still read as bags at 22 blocks, or as a brown block
    await aim(cx+16, cz, cy+1.6, cx, cz, cy); await sleep(1400);
    fs.writeFileSync(path.join(OUT,'chainlink-far.png'), await page.screenshot());
    ...
    fs.writeFileSync(path.join(OUT,'chainlink-crest.png'), await page.screenshot());
    ...
    await aim(cx+5.5, cz, cy+0.4, cx, cz, cy); await sleep(1600);
    fs.writeFileSync(path.join(OUT,'chainlink-night.png'), await page.screenshot());
    // IN THE HAND, with the hotbar icon in the same frame
    await page.evaluate('__hc.setTime(0.42)'); await sleep(1400);
    await page.evaluate('(()=>{ __hc.cmdRun("/give chainlink 12"); __hc.hold("chainlink"); })()'); await sleep(2400);
    fs.writeFileSync(path.join(OUT,'chainlink-held.png'), await page.screenshot());

// The tree rework — census of the species mix, and one shot of the lowland wood and one of the treeline.
// node bench/tmp-tree-shots.mjs
...
(async()=>{
  ...
  try{
    ...
    await sleep(2500);
    console.log('census   ' + JSON.stringify(await page.evaluate('__hc.treeCensus(160,4)')));
    console.log('here     ' + JSON.stringify(await page.evaluate('__hc.treeGates(__hc.probe().x,__hc.probe().z)')).slice(0,200));
    ...
    if(spot){
      ...
      fs.writeFileSync(path.join(ROOT,'bench','results','tree-wood.png'), await page.screenshot());
      console.log('wrote tree-wood.png');
    }
    await browser.close();

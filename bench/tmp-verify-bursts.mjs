...
(async()=>{
  ...
  try{
    ...
    await sleep(1000);
    const b = await ev(page,`__hc.burstTest()`);
    const pp=await ev(page,`__hc.pos()`); await ev(page,`__hc.tp(${pp.x},${pp.y+4},${pp.z},${pp.yaw},-0.5)`); await sleep(400);
    await page.screenshot({ path: path.join(OUT,'vis-bursts.png') });

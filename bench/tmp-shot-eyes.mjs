...
(async () => {
  ...
  try {
    ...
    await sleep(1500);
    await page.evaluate(`__hc.boss({park:true, dist:30, up:8})`);
    await sleep(9000);
    await page.evaluate(`__hc.aimEye()`); await sleep(400);
    await page.evaluate(`__hc.cam && __hc.cam({pitch:0.12})`); await sleep(300);
    await page.screenshot({ path: path.join(OUT,'eyes-front.png') });
    const e=await page.evaluate(`__hc.eyes()`); console.log(JSON.stringify({n:e.spheres&&e.spheres.length}));

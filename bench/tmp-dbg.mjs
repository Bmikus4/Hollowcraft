...
// identical deterministic camera in every mode (spawn is deterministic); look slightly down toward inland structures/terrain
async function shot(browser, port, tag, q){
  ...
  await sleep(6000);
  // identical deterministic camera to the known-good spawn shot (sp-day-1): the beach + forested hill with the dark shadowed faces
  await page.evaluate(`__hc.aimAt(player.pos.x+Math.cos(2.1)*30, player.pos.y+2, player.pos.z+Math.sin(2.1)*30)`).catch(()=>{});
  await sleep(600);
  await page.screenshot({ path: path.join(OUT, `dbg-${tag}.png`) });
  await page.close();
  return { tag, errors };
}
(async () => {
  ...
  try {
    ...
    r.push(await shot(browser, port, 'albedo', '&albedo=1'));
    r.push(await shot(browser, port, 'sky',    '&dbg=sky'));
    r.push(await shot(browser, port, 'nrm',    '&dbg=nrm'));

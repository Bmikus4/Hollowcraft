...
(async()=>{
  ...
  try{
    ...
    const IDS=[TIER+'_pickaxe',TIER+'_axe',TIER+'_shovel',TIER+'_sword','wooden_spear','rusty_spear'];
    for(const id of IDS){ await page.evaluate(`__hc.hold(${JSON.stringify(id)})`); await sleep(500);
      fs.writeFileSync(path.join(ROOT,'bench','results','hand-'+id+'.png'), await page.screenshot());
      console.log('  hand-'+id+'.png'); }
    // the whole hotbar in one strip, at the size a player actually sees it — a 3D-baked tool icon next to two
    // flat atlas tiles (the ingot and the bread), which is the comparison that says whether it reads or not
    await page.evaluate(`__hc.fillHotbar(${JSON.stringify([TIER+'_pickaxe',TIER+'_axe',TIER+'_shovel',TIER+'_sword','wooden_spear','rusty_spear','iron_ingot','bread','torch'])})`);
    await sleep(700);
    fs.writeFileSync(path.join(ROOT,'bench','results','hotbar-tools.png'), await page.screenshot({ clip:{x:195,y:520,width:510,height:70} }));
    console.log('  hotbar-tools.png');
    await browser.close();

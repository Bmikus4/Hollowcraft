...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(`window.__hcBR.enter()`); await sleep(6500);

    const g = await page.evaluate(`window.__hcBRX.goreScan(120)`);
    console.log('gore scan', JSON.stringify(g));
    T('a gore room is ALWAYS on tile, never carpet or boards', g.goreNonTile===0, {goreNonTile:g.goreNonTile, gore:g.gore});
    T('blood rooms land near 1%', g.bloodPct>0.55 && g.bloodPct<1.6, {bloodPct:g.bloodPct, n:g.rooms});
    T('gore rooms land near 0.1% of all rooms', g.gorePct>0.04 && g.gorePct<0.22, {gorePct:g.gorePct, gore:g.gore, n:g.rooms});
    T('the sample is big enough to mean something', g.rooms>4000, {rooms:g.rooms});

    // and it must actually build: force one, count the triangles it adds
    const before = await page.evaluate(`window.__hcBRX.envStats()`);
    const room = await page.evaluate(`window.__hcBRX.forceGore()`); await sleep(1500);
    const after = await page.evaluate(`window.__hcBRX.envStats()`);
    console.log('forced gore room', JSON.stringify(room), 'tris', before.tris, '->', after.tris);
    T('a gore room adds real geometry', after.tris-before.tris>400, {added:after.tris-before.tris});
    T('no chunk failed to build with gore present', (after.errs||[]).length===0, (after.errs||[]).slice(0,2));
    await page.evaluate(`__hc.aim(false)`); await sleep(1200);
    await page.screenshot({ path: path.join(OUT,'br-gore.png') });

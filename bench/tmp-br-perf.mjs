...
(async()=>{
  ...
  try{
    ...
    // 1. SPAWNING THE PORTAL
    const tPortal = await page.evaluate(`(()=>{ const t0=performance.now(); window.__hcBR.portal? window.__hcBR.portal() : null; return performance.now()-t0; })()`).catch(()=>null);
    await sleep(2500);
    const fpsPortal=[]; for(let i=0;i<5;i++){ await sleep(500); fpsPortal.push((await page.evaluate(`__hc.st()`)).fps); }
    console.log('B. portal spawn call ms:', tPortal, ' fps with portal up:', JSON.stringify(fpsPortal));

    // 2. WALKING THROUGH (first entry: generate + build everything)
    const tEnter = await page.evaluate(`(()=>{ const t0=performance.now(); window.__hcBR.enter(); return performance.now()-t0; })()`);
    await sleep(6000);
    const st=await page.evaluate(`window.__hcBRX.envStats()`);
    const fpsIn=[]; for(let i=0;i<5;i++){ await sleep(500); fpsIn.push((await page.evaluate(`__hc.st()`)).fps); }
    console.log('C. brEnter() ms:', Math.round(tEnter), ' meshes:', st.meshes, ' tris:', st.tris, ' fps inside:', JSON.stringify(fpsIn));

    // 3. CROSSING INTO NEW CHUNKS — the number that matters most
    const cross=[];
    for(const [gx,gz] of [[1,0],[2,0],[2,1],[3,1],[3,2]]){
      const ms = await page.evaluate(`(()=>{ const t0=performance.now(); window.__hcBRX.walkTo(${gx},${gz}); return performance.now()-t0; })()`);
      cross.push(Math.round(ms)); await sleep(900); }
    console.log('D. chunk crossing ms:', JSON.stringify(cross));

    // where does the crossing time go?
    const split = await page.evaluate(`(()=>{ const c=window.__hcBRX.crossSplit? window.__hcBRX.crossSplit(9,9) : null; return c; })()`).catch(()=>null);
    console.log('E. crossing breakdown:', JSON.stringify(split));
    await browser.close();

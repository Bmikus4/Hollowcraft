...
(async()=>{ const port=await freePort();
  ...
  try{ await waitHttp('http://127.0.0.1:'+port+'/index.html');
    ...
    await sleep(1500);
    await page.evaluate("(()=>{ __hc.freeze(true,true); __hc.studio(true); __hc.frame({dist:10, ang:0, state:'HUNT', crawl:1, adv:8, committed:true, aim:1.6}); __hc.spin(90); })()");
    await sleep(1200);
    await page.screenshot({path:'bench/results/gallop-check2.png'});

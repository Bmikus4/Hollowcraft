...
(async () => {
  ...
  try {
    ...
    await sleep(3500);
    const carve = await page.evaluate(`__hc.qaAtlasCarve()`);
    const use = await page.evaluate(`__hc.qaAtlasUse()`);
    await sleep(2200);
    const s = carve.sample;
    const restored = await page.evaluate(`__hc.qaBlock(${s[0]},${s[1]},${s[2]})`);
    console.log(JSON.stringify({ carve, use, restoredBlock:restored, matchesOriginal: restored===carve.wasBlock, errors }));

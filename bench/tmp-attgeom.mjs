...
(async()=>{ const port=await freePort();
  ...
  let b; try{ const base='http://127.0.0.1:'+port; await waitHttp(base+'/index.html');
    ...
    for(const g of ['ar15','ak','smg','shotgun_long']){
      for(const a of ['red_dot','holo_sight','optic_scope']){
        await p.evaluate(`__hc.hold(${JSON.stringify(g)})`); await sleep(300);
        const r=await p.evaluate(`(()=>{__hc.cmdRun('/give ${a} 1'); return __hc.attFit('optic',${JSON.stringify(a)});})()`);
        console.log(g,a,JSON.stringify(r));
      }
    }

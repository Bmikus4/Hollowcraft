...
(async () => {
  ...
  try {
    ...
    // teleport to high inland ground so the village has a valid site, let chunks stream
    await page.evaluate(`__hc.tpExact(${(st.sx||0)+120}, ${(st.sz||0)+120})`);
    await sleep(9000);
    ...
    console.log(JSON.stringify({ intactCount:v.intactCount, cottages:v.cottages, spot:v.spot, errors }));

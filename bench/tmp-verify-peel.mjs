...
(async()=>{
  ...
  try{
    ...
    await ev(page,`__hc.setTime(0.4)`); const pp=await ev(page,`__hc.pos()`); await ev(page,`__hc.tp(${pp.x},${pp.y},${pp.z},${pp.yaw},-0.55)`); await sleep(600);
    const p0 = await ev(page,`__hc.peel({begin:0.7, rate:0.5})`);
    await sleep(3500);
    const p1 = await ev(page,`__hc.peel({})`);
    await page.screenshot({ path: path.join(OUT,'vis-peel.png') });
    await sleep(3000);
    const p2 = await ev(page,`__hc.peel({})`);
    await page.screenshot({ path: path.join(OUT,'vis-peel2.png') });
    console.log(JSON.stringify({ pageErrors:errors, p0, p1, p2 }, null, 1));
    await browser.close();

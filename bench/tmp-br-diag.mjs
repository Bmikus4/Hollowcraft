...
(async()=>{
  ...
  try{
    ...
    const fx = await page.evaluate(`window.__hcBRX.standUnderFixture()`); await sleep(1400);
    console.log('under fixture', JSON.stringify(fx));
    const shot = await page.screenshot({ path: path.join(OUT,'br-underlight.png') });
    console.log('luminance', JSON.stringify(pngStats(shot)));
    await browser.close();

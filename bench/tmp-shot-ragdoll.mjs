...
(async () => {
  ...
  try {
    ...
    await sleep(2500);
    const k = await page.evaluate(`__hc.corpseKill(false, 15)`);    // drop a wretch corpse from 15 blocks UP → test the fall
    const fall=[];
    for(let i=0;i<9;i++){ const s=await page.evaluate(`__hc.ragState()`); fall.push(s.map(r=>r.y+(r.settled?'S':r.landed?'L':'.'))); await sleep(400); }
    const s0=fall[0], s1=await page.evaluate(`__hc.ragState()`);
    // look at the corpse
    await page.evaluate(`__hc.aimAt(${k.x}, ${k.gy}, ${k.z})`); await sleep(400);
    await page.screenshot({ path: path.join(OUT,'ragdoll-wretch.png') });
    console.log(JSON.stringify({ kill:k, atSpawn:s0, afterSettle:s1, errors }));

...
(async () => {
  ...
  try {
    ...
    await sleep(3500);
    const on = await page.evaluate(`__hc.postfx()`);
    // whip the camera to trigger motion blur, screenshot mid-turn
    await page.evaluate(`__hc.pitch && __hc.pitch(0)`);
    for(let i=0;i<6;i++){ await page.evaluate(`(()=>{ try{ __hc.aimAt(player.pos.x+Math.cos(${i})*20, player.pos.y+3, player.pos.z+Math.sin(${i})*20); }catch(e){} })()`).catch(()=>{}); }
    await page.screenshot({ path: path.join(OUT,'postfx-mb-on.png') });
    const off = await page.evaluate(`__hc.setMB(false)`); await sleep(400);
    const offState = await page.evaluate(`__hc.postfx()`);
    console.log(JSON.stringify({ postfxOn:on, afterToggleOff:offState, errors }));

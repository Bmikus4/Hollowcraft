...
(async()=>{
  ...
  try{
    ...
    await ev(page,`__hc.setTime(0.4)`); const pp=await ev(page,`__hc.pos()`); await ev(page,`__hc.tp(${pp.x},${pp.y},${pp.z},${pp.yaw},-0.5)`); await sleep(700);
    const w = await ev(page,`__hc.whitenTest()`);
    await sleep(400); await page.screenshot({ path: path.join(OUT,'vis-whiten.png') });
    console.log(JSON.stringify({ pageErrors:errors, w }, null, 1));
    await browser.close();

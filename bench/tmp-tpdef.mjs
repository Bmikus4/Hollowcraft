...
(async () => {
  ...
  try {
    ...
    await sleep(2500);
    await page.evaluate(`__hc.treeprobe()`); await sleep(9000);   // elevated open-air view over the forest (same framing as the pre-change baseline)
    await page.screenshot({ path: path.join(OUT, `tpdef-after.png`) });
    console.log(JSON.stringify({ st: await page.evaluate(`__hc.st()`), errors }));

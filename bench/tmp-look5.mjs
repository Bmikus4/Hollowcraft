...
try{
  ...
  const t = (c.per.sunflower_wild && c.per.sunflower_wild.at) || (c.per.tree_flower && c.per.tree_flower.at);
  if(t){
    await W.ev(`(function(){ __hc.tpAt(${t[0]}+7, ${t[1]}+3, ${t[2]}+7);
      const p=__hc.pos(); H.cam({yaw:Math.atan2(-(${t[0]}-p.x), -(${t[2]}-p.z)), pitch:-0.12}); })()`);

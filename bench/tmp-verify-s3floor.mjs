...
(async()=>{
  ...
  try{
    ...
    await ev(page,`__hc.lock(true)`); await sleep(800);
    const start = await ev(page,`__hc.voidFloorTest()`);
    await sleep(2500);
    const landed = await ev(page,`__hc.voidFloorState()`);            // rests ON the flat floor (restDelta ~0, onGround)
    await ev(page,`__hc.key('KeyW',true)`); await sleep(2000); await ev(page,`__hc.key('KeyW',false)`);
    const roamed = await ev(page,`__hc.voidFloorState()`);            // still on the flat floor after roaming (no fall-through, no tile wall)
    await page.screenshot({ path: path.join(OUT,'vis-void-floor.png') });
    console.log(JSON.stringify({ pageErrors:errors, start, landed, roamed }, null, 1));
    await browser.close();

// Verify the structure/well leaves fix: force-build the well, count tree blocks left on its pad (expect ~0).
...
(async()=>{
  ...
  try{
    ...
    await sleep(1500);
    const centered = await page.evaluate(`__hc.floodTest(false)`);
    const overhang = await page.evaluate(`__hc.floodTest(true)`);
    console.log(JSON.stringify({ pageErrors:errors, centered, overhang }, null, 1));
    await browser.close();

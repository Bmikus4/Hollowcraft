...
const ALLOWED = new Set(['p','b','bb','tree','drop','drops','dpick','an','w','own','grab','rescue','sync','time','chest','dmg','bossdead','hb','shot','hw','han','seraphSay','bosscine','wretchdead','mine','cmd']);   // relay only known message types ('bb'/'drops' = per-frame batches; dmg/bossdead/hb/shot/hw/han = boss+Herobrine+gun sync; seraphSay/bosscine = the seraph's voice + arrival cutscene; 'mine' = a landmine's blast for the show; 'cmd' = a console command aimed at one player, e.g. /give — it carries `to` and the relay already routes those to that client alone)

...
| **P5** | Stop the portal re-rendering the world at 250 fps | `PERF.portalHz` (baseline 0 → 120) | a second full scene render every frame within 30 m. P1/P2 had already cut it 11.48 → 2.94 ms CPU, but that is still 41 % of the budget for one prop | fewer duplicate frames without visible staleness | **−1.48 ms, 5 of 5 pairs, draws 755 → 421, frames >12 ms −61.** Portal refresh 43 → **91 Hz** standing still (the game runs 61 → 197 fps, so it updates *more* often) | **DONE** |
| **P6** | One shared streaming budget + admission control | `PERF.streamBudgetMs` (baseline 0 → 8) | measured: gen averages 3.2–4.9 ms against a 2.5 ms slice with a tail to 24 ms, and gen+mesh had separate budgets so a frame overran both (`gen 18.0 + mesh 6.4`) | stop the two halves compounding | **frames >12 ms −60.5 (B2o) / −114 (B3o), B3o p99 −3.61 ms, max −6.89 ms.** Medians unchanged, which is correct. Throughput identical: 242 chunks, 1 125 gens, 1 884 meshes either way | **DONE** |
| **P7** | Bound the `BR.gen` cache | `PERF.brGenCacheMax` (baseline 0 → 512) | I claimed the 231 MB heap was this cache growing without limit | flat heap over the V7 soak | **the claim was wrong — `BR.gen` peaks at 156 entries ≈ 6 MB, so the cap never fires.** A/B correctly shows nothing: +0.07 ms, 1 of 4 pairs | **DONE as insurance, not as the heap fix.** The real heap source is unidentified and needs a snapshot |
| **P8** | Entity-tick spikes | `PERF.brainScanSlice` (baseline 0 → 400) + edit-key cache | bisected to `detectBase`, which sweeps every block edit in the world every 3 s. `Object.keys(edits)` was the cost, not the loop | overworld under 12 ms sustained | **detectBase worst frame 8.10 → 0.13 ms (62×).** B1o and B5o now pass C1 **and** C2 | **DONE** |
| **P9** | Preload gate | `PERF.preloadSliceMs` (baseline 5 → 30) | my "6.9 s is icon baking" claim was wrong — measured, the whole bake is 944 ms and the readback is 11 % of it. The waste was 5 ms of work per full-game-render frame | shorter load | **first interactive 6 775 → 5 884 ms, −13 %**, three cold loads each, non-overlapping | **DONE** |
